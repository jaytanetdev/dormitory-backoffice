...
export interface InvoiceDto { id:string;number:string;total:number|string;status:string;dueDate:string;room:{number:string};contract:{resident:{fullName:string}};items:{type?:string;code?:string;amount:number|string;quantity?:number|string;unitPrice?:number|string;metadata?:unknown}[] }
