...
export interface InvoiceDto { id:string;number:string;total:number|string;status:string;dueDate:string;createdAt?:string;period?:{year:number;month:number};room:{id?:string;number:string};contract:{id?:string;resident:{fullName:string}};items:{type?:string;code?:string;amount:number|string;quantity?:number|string;unitPrice?:number|string;metadata?:unknown}[] }
export interface PaymentDto { id:string;amount:number|string;createdAt:string;status:string;paidAt?:string;invoice:{id?:string;number?:string;dueDate?:string;room:{number:string};contract:{resident:{fullName:string}}};slip:{fileUrl:string;fileName?:string|null;mimeType?:string|null}|null }
export interface ReceiptDto { id:string;number:string;amount:number|string;lateFee:number|string;totalAmount:number|string;issuedAt:string }
