...
export default function BillsPage() {
  ...
  return <>
    ...
    <section className="billing-overview"><div><span className="eyebrow">รอบบิลที่กำลังทำ</span><h2>{new Intl.DateTimeFormat("th-TH", { month: "long", year: "numeric" }).format(new Date(billingYear, billingMonth - 1, 1))}</h2><p>เช็กห้องที่ทำแล้วก่อนเริ่มออกบิลรอบนี้</p></div><div className="billing-period-fields"><label>เดือน<Select value={String(billingMonth)} onValueChange={(value) => setBillingMonth(Number(value))} options={Array.from({ length: 12 }, (_, index) => ({ value:String(index + 1), label:new Intl.DateTimeFormat("th-TH", { month: "long" }).format(new Date(2026, index, 1)) }))} /></label><label>ปี<input type="number" value={billingYear} onChange={(event) => setBillingYear(Number(event.target.value))} /></label></div><div className="billing-progress"><strong>{monthBills.length} <small>/ {activeContracts.length}</small></strong><span>ห้องที่ออกบิลแล้ว</span><div><i style={{ width: `${activeContracts.length ? Math.min(100, monthBills.length / activeContracts.length * 100) : 0}%` }} /></div><em>{missingContracts.length ? `เหลืออีก ${missingContracts.length} ห้อง` : "ครบทุกห้องแล้ว"}</em></div></section>
