...
import { defaultBillingDueDate } from "@/lib/billing-utils";
...
type Contract = { id: string; status: string; startDate: string; billingDay?: number; monthlyRent: number | string; room: { id: string; number: string }; resident: { fullName: string } };
...
export default function BillsPage() {
  ...
  const visible = useMemo(() => items.filter((bill) => (!query || `${bill.number} ${bill.room.number} ${bill.contract.resident.fullName}`.toLowerCase().includes(query.toLowerCase())) && (roomFilter === "all" || (roomFilter === "done" && monthBills.some((item) => item.id === bill.id)))), [items, monthBills, query, roomFilter]);
  ...
  const defaultDueDate = selectedContract ? defaultBillingDueDate(billingYear, billingMonth, selectedContract.billingDay ?? 1, invoiceSettings.data?.invoiceDueDays ?? 5) : "";
  ...
  async function create(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!dueDate) { setBusy(false); setError("กรุณาเลือกวันครบกำหนด"); return; }
    ...
    const result = await apiMutation<InvoiceDto>("/invoices", { branchId, periodId: period.data.id, contractId, dueDate, number: `INV-${year}${String(month).padStart(2, "0")}-${selectedContract.room.number}-${Date.now().toString().slice(-5)}`, discount: 0, items: lineItems, meterReadings });
    setBusy(false);
    if (!result.ok) { setError(result.message); return; }
    const mappedItems = lineItems.map((item) => ({ ...item, amount: item.quantity * item.unitPrice }));
    setItems((current) => [{ ...result.data, status: "DRAFT", period: { year, month }, room: { id: selectedContract.room.id, number: selectedContract.room.number }, contract: { id: selectedContract.id, resident: { fullName: selectedContract.resident.fullName } }, items: mappedItems }, ...current]);
    setOpen(false); setContractId("");
    void queryClient.invalidateQueries({ queryKey: ["api"] });
  }

  async function issue(invoiceId: string) {
    setIssuingId(invoiceId); setError(null); setNotice(null);
    const result = await apiMutation<{ invoice: InvoiceDto; notification?: { status?: string } }>(`/invoices/${invoiceId}/issue`, {});
    setIssuingId(null);
    if (!result.ok) { setError(result.message); return; }
    setItems((current) => current.map((invoice) => invoice.id === invoiceId ? { ...invoice, status: "ISSUED" } : invoice));
    setNotice(result.data.notification?.status === "SENT" ? "ออกบิลและส่ง LINE แล้ว" : "ออกบิลแล้ว แต่ยังส่ง LINE ไม่สำเร็จ กรุณาตรวจสอบการเชื่อม LINE และแจ้งผู้เช่าผ่านแชท");
    void queryClient.invalidateQueries({ queryKey: ["api"] });
  }
  ...
  return <>
    ...
    {open && <div className="modal-backdrop"><section className="modal bill-modal"><div className="modal-head"><div><h2>สร้างใบแจ้งหนี้รายเดือน</h2><p className="subtitle">เลขครั้งก่อนดึงจากประวัติมิเตอร์ล่าสุดของห้อง</p></div><button className="icon-button" onClick={() => setOpen(false)}>×</button></div>
      <form onSubmit={create}>
        <p className="auto-due-date">วันครบกำหนดจะคำนวณอัตโนมัติจากวันเก็บเงินในรอบเดือนที่เลือก + {invoiceSettings.data?.invoiceDueDays ?? 5} วัน{defaultDueDate ? ` · ${new Date(`${defaultDueDate}T12:00:00`).toLocaleDateString("th-TH")}` : ""}</p>
    ...
    <div className="toolbar"><input className="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="ค้นหาเลขบิล ห้อง หรือผู้เช่า" /><Select className="filter" value={roomFilter} onValueChange={(value) => setRoomFilter(value as typeof roomFilter)} options={[{ value:"missing", label:`ยังไม่ออกบิล · ${missingContracts.length}` }, { value:"done", label:`มีบิลแล้ว · ${monthBills.length}` }, { value:"all", label:"ทุกใบแจ้งหนี้" }]} /></div>
    {roomFilter === "missing" ? <div className="table-wrap"><table className="data-table"><thead><tr><th>ห้อง</th><th>ผู้เช่า</th><th>ดำเนินการ</th></tr></thead><tbody>{missingContracts.filter((contract) => `${contract.room.number} ${contract.resident.fullName}`.toLowerCase().includes(query.toLowerCase())).map((contract) => <tr key={contract.id}><td>{contract.room.number}</td><td>{contract.resident.fullName}</td><td><button className="button" onClick={() => { setContractId(contract.id); setOpen(true); }}>สร้างบิล</button></td></tr>)}</tbody></table>{!missingContracts.length && <p className="empty-state">สร้างบิลครบแล้วในรอบนี้</p>}</div> : <div className="table-wrap"><table className="data-table"><thead><tr><th>เลขที่</th><th>ห้อง / ผู้เช่า</th><th>ค่าเช่า</th><th>น้ำ</th><th>ไฟ</th><th>ยอดรวม</th><th>สถานะ</th><th>ดำเนินการ</th></tr></thead><tbody>{visible.map((bill) => <tr key={bill.id}><td className="mono">{bill.number}</td><td><strong>{bill.room.number}</strong><br /><small>{bill.contract.resident.fullName}</small></td><td>฿{amount(bill, "RENT").toLocaleString()}</td><td>฿{amount(bill, "WATER").toLocaleString()}</td><td>฿{amount(bill, "ELECTRIC", "ELECTRICITY").toLocaleString()}</td><td><strong>฿{Number(bill.total).toLocaleString()}</strong></td><td><Status>{statuses[bill.status] ?? bill.status}</Status></td><td>{bill.status === "DRAFT" ? <button className="room-invite-button" disabled={issuingId === bill.id} onClick={() => void issue(bill.id)}>{issuingId === bill.id ? "กำลังส่ง…" : "ออกบิล + แจ้ง LINE"}</button> : <span className="table-muted">—</span>}</td></tr>)}</tbody></table>{!visible.length && !invoices.loading && <p className="empty-state">ไม่พบใบแจ้งหนี้ตามตัวกรอง</p>}</div>}
  </>;
}
